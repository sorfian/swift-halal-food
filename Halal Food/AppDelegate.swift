...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    lazy var persistentContainer: NSPersistentContainer = {
        ...
        */
        let container = NSPersistentContainer(name: "HalalFood")
...
extension AppDelegate: UNUserNotificationCenterDelegate {
    func userNotificationCenter(_ center: UNUserNotificationCenter, didReceive response: UNNotificationResponse, withCompletionHandler completionHandler: @escaping () -> Void) {
        if response.actionIdentifier == "halalfood.makeReservation" {
            print("Make a reservation...")
            
            if let phone = response.notification.request.content.userInfo["phone"] {
                let telephoneURL = "tel://\(phone)"
                if let url = URL(string: telephoneURL) {
                    if UIApplication.shared.canOpenURL(url) {
                        print("calling \(telephoneURL)")
